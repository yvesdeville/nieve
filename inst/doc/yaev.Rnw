...
\item \fbox{NT} Use only the formula for $\xi \neq 0$ i.e., {``do
  ...
  will never come to the case $\xi = 0$ exactly!  If the distribution
  is estimated with no possible constraint or initialisation with
  $\xi = 0$, there will be no problem.
