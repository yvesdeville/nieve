...
$\epsilon = \text{1e-4}$ leads to an error of order~$\text{1e-12}$
with is suitable for practical. This kind of approximation is used in
some codes of the \textbf{revdbayes} package by Paul Northrop, see the
code if the \code{dgev} and \code{pgev} functions on
