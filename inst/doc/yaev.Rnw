...
for~$\partial^2_{\xi,\xi} q$ or a crossed derivative such
...
small: $\text{1e-4}$ or $\text{1e-5}$, not much smaller. This method
is used in some codes of the \textbf{revdbayes} package by Paul
Northrop.
