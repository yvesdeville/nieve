...
As an example, consider the quantile function of the Generalised Pareto
...
\begin{equation}
  ...
  \right.
\end{equation}
The fraction $[(\lambda_u w)^\xi - 1]/\xi$ of the first equation is to
...
\begin{equation}
  ...
  \right.
\end{equation}
The second equation becomes $\lambda = w^{-1}$ for $\xi^\star = 0$.
...
parametrically or non-parametrically.  Anyway, for non-stationary
models it is often useful to transform one of the two parameterisation
into the other. If a Poisson-GP model is used, we may be interested in
the GEV reference distribution conditional on a given value
$\m{x}$. When instead a PP model is used, it may be useful to
investigate the relation of the implied rate $\lambda$ with the
...
for $\xi = 0$ and $\xi^\star = 0$ is coped with by using a
second-order Taylor approximation. As often required when coping with
...
in the provided vectors such as \verb@lambda@ correspond to a value
$\m{x}$ of the covariates and most often the threshold is then also
chosen as depending on the covariates.
