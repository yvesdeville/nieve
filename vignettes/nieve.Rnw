...
Generalised Extreme Value (GEV) distributions \cite{Coles_ISMEV} are
...
function of the Generalised Pareto distribution with shape $\xi$ and
unit scale
\begin{equation}
\label{eq:Quant}
q(p) = \begin{cases}
...
$\xi$ are often used in the initialisation of the Maximum-Likelihood
(ML) optimisation.
...
optimisation tasks required in EV analysis. These tasks of course
...
\href{https://github.com/yvesdeville/nieve/blob/main/inst/doc/PoisGP2PP.pdf}{PoisGP2PP.pdf}
and
...
also provides workable approximations for the case $\xi \approx 0$. We
used the \href{https://maxima.sourceforge.io/}{Maxima Computer Algebra
  System}~\cite{Maxima} along with the
